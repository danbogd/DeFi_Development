...
contract CombToken is ERC20("Combine.finance", "COMB"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "COMB::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "COMB::delegateBySig: invalid nonce");
        require(now <= expiry, "COMB::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "COMB::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying COMBs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "COMB::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable {
    ...
    uint256 public combLastBlock = 7.5 ether; // 75 tokens
    ...
    function getPendingReward(uint256 _fromBlock, uint256 _combLastBlock) public view returns (uint256, uint256) {
        uint256 blocksPassed = block.number.sub(_fromBlock);
        uint256 reward = 0;
        while (blocksPassed > 0) {
            reward = reward.add(_combLastBlock);
            // Rewards decrease by 0.1% on every block
            _combLastBlock = _combLastBlock.mul(999).div(1000);
            blocksPassed = blocksPassed.sub(1);
        }
        return (reward, _combLastBlock);
    }
    ...
    function pendingComb(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            (uint256 pendingReward, ) = getPendingReward(pool.lastRewardBlock, combLastBlock);
    ...
    function updatePool(uint256 _pid) public {
        ...
        (uint256 pendingReward, uint256 newCombLastBlock) = getPendingReward(pool.lastRewardBlock, combLastBlock);
        uint256 combReward = pendingReward.mul(pool.allocPoint).div(totalAllocPoint);
        comb.mint(address(this), combReward);
        pool.accCombPerShare = pool.accCombPerShare.add(combReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
        combLastBlock = newCombLastBlock;
    }
